...
# RL on a 10K eval_type-balanced subset of train_rl_public.parquet.
# Build the subset first:
#   python scripts/utils/build_balanced_rl_subset.py --target-size 10000
#
# Override precedence for the SFT init: SFT_CKPT > (SFT_DIR + SFT_STEP) > (SFT_DIR + latest_checkpointed_iteration.txt)
...
TRAIN_FILE="${TRAIN_FILE:-$REPO/data/train_rl_public_balanced_10000.parquet}"
[[ -f "$TRAIN_FILE" ]] || { echo "error: train parquet not found: $TRAIN_FILE — run scripts/utils/build_balanced_rl_subset.py first" >&2; exit 1; }
echo "Using train parquet: $TRAIN_FILE"

PER_GPU=8
SAVE_PATH="${SAVE_PATH:-$REPO/outputs/rl_4gpu_balanced_10k}"
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=rl_4gpu_balanced_10k \
