...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            import json
            def _parse_reward_model(rm):
                if isinstance(rm, str):
                    rm = json.loads(rm)
                return rm.get("ground_truth", None) if isinstance(rm, dict) else None
            ground_truths = [
                _parse_reward_model(item.non_tensor_batch.get("reward_model", {})) for item in test_batch
            ]
        ...
        self._maybe_log_val_generations(inputs=sample_inputs, outputs=sample_outputs, scores=sample_scores, ground_truths=sample_gts)
