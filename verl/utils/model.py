...
def extract_multi_modal_inputs(
    ...
    Handles dynamic padding for variable length inputs (e.g. time-series, audio).

    This is customized for timeseries data. Other usage, beware and check.
    """
    multi_modal_inputs = {}
    ...
    for inputs in selected_batch_data:
        # Handle verl's NonTensorData wrapper if it exists
        if hasattr(inputs, 'data'):
            inputs = inputs.data
            
        # Mixed pure text and multi-modal dataset handling
        if inputs is None:
            continue
    ...
    for key, values in multi_modal_inputs_collected.items():
        ...
        if has_image_bound:  # minicpm-o specific logic
            multi_modal_inputs[key] = values
        else:
            # --- Dynamic Padding Logic ---
            # values is a list of tensors, e.g., [(1, 248, 1), (1, 512, 1)]
            
            # 1. Determine maximum size for each dimension (skipping batch dim 0)
            ref_dim = values[0].dim()
            max_shape = list(values[0].shape)
            
            for v in values:
                for i in range(1, ref_dim):
                    if v.shape[i] > max_shape[i]:
                        max_shape[i] = v.shape[i]
            
            padded_values = []
            for v in values:
                # 2. Check if padding is needed for this specific tensor
                if list(v.shape[1:]) == max_shape[1:]:
                    padded_values.append(v)
                    continue
                
                # 3. Construct padding tuple for F.pad
                # F.pad args are: (last_dim_left, last_dim_right, 2nd_last_left, ...)
                pad_args = []
                for i in range(ref_dim - 1, 0, -1): # Iterate backwards from last dim to dim 1
                    diff = max_shape[i] - v.shape[i]
                    pad_args.extend([0, diff]) # Pad right side with 0
                
                # 4. Pad with 0 (Matches processor's np.pad default and model's mask logic)
                padded_values.append(F.pad(v, pad_args, value=0))
            
            multi_modal_inputs[key] = torch.cat(padded_values, dim=0)
