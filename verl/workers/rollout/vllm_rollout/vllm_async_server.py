...
class vLLMHttpServerBase:
    ...
    def __init__(
        ...
        # Resolve </think> token ID for ThinkGreedy logits processor
        try:
            from transformers import AutoTokenizer
            _tmp_tokenizer = AutoTokenizer.from_pretrained(self.model_config.local_path, trust_remote_code=True)
            self.think_end_token_id = _tmp_tokenizer.encode("</think>", add_special_tokens=False)[-1]
            logger.info(f"ThinkGreedy enabled: </think> mapped to token ID {self.think_end_token_id}")
            os.environ["VERL_THINK_TOKEN_ID"] = str(self.think_end_token_id)
        except Exception as e:
            logger.warning(f"Could not resolve </think> token ({e}). ThinkGreedy logits processor will be skipped.")
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        if self.config.get("limit_images", None):
            limit_mm["image"] = self.config.get("limit_images")
        limit_mm["timeseries"] = self.config.get("limit_timeseries", 50)

        if limit_mm:
            engine_kwargs["limit_mm_per_prompt"] = limit_mm

        # Disable MM processor cache to avoid P0/P1 cache desync with data parallelism.
        # With DP, the sender cache (P0) is shared but each DP rank has its own receiver
        # cache (P1). P0 may skip sending data for a hash it already sent, but the request
        # can land on a different DP rank that never received it, causing an AssertionError.
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        # Inject ThinkGreedy logits processor (greedy decoding after </think>)
        if self.think_end_token_id is not None:
            from verl.workers.rollout.vllm_rollout.utils import ThinkGreedyLogitsProcessor
            if getattr(vllm_config.model_config, "logits_processors", None) is None:
                vllm_config.model_config.logits_processors = []
            vllm_config.model_config.logits_processors.append(ThinkGreedyLogitsProcessor)
    ...
    async def generate(
        ...
        timeseries_data: Optional[list[Any]] = None,
    ) -> TokenOutput:
        ...
        if timeseries_data is not None:
            ...
            logger.warning(f"vLLMHttpServer.generate: no timeseries_data for request {request_id}")
