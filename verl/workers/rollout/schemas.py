...
class AsyncRolloutRequest(BaseModel):
    ...
    def initialize_request(cls, values):
        ...
        if not values.get("multi_modal_keys"):
            values["multi_modal_keys"] = ["image", "video", "timeseries"]
    ...
    def _handle_apply_chat_template(
        ...
        if isinstance(processing_class, PreTrainedTokenizer) or isinstance(processing_class, PreTrainedTokenizerFast):
            ...
            timeseries = timeseries if len(timeseries := multi_modal_data.get("timeseries", [])) > 0 else None

            # Build kwargs dynamically based on what's available
            ...
            if timeseries is not None:
                processor_kwargs["timeseries"] = timeseries
    ...
    def add_tool_response_messages(
        ...
        for content in contents:
            if content.is_text_only():
                ...
                if hasattr(content, 'timeseries') and content.timeseries:
                    content_list.extend([{"type": "timeseries"} for _ in content.timeseries])
                    delta_multi_modal_data["timeseries"].extend(content.timeseries)
                if content.text:
